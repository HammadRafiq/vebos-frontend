...
import {
  ...
  signInCreatorSchema,
} from "./sign-in-company.schema";
import toast from "react-hot-toast";
import { useBrandLoginMutation } from "@root/src/service/auth";
import LoadingComponent from "@components/loading";
import { useRouter } from "next/navigation";

export const SignInCompanyForm = () => {
  const { push } = useRouter();
  const [mutation, { isLoading }] = useBrandLoginMutation();
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className=" flex justify-between items-center">
        ...
        <Button
          onClick={() => push("sign-up-company")}
          variant="destructive"
          type="button"
          className="mt-[48px] w-fit"
        >
          Sign Up as Company/Brand
        </Button>
      </div>
    </FormProvider>
  );
